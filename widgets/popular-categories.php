...
// Lists most popular categories in a widget (Robin Dalton - http://bit.ly/qGeSZu)
...
class Most_Popular_Categories_Widget extends WP_Widget {
  ...
  function Most_Popular_Categories_Widget() {
    /* Widget settings. */
    $widget_ops = array('classname' => 'popularcategories', 'description' => __('Display popular categories.', 'youare'));

    /* Widget control settings. */
    $control_ops = array('width' => 250, 'height' => 350, 'id_base' => 'popcat-widget');

    /* Create the widget. */
    $this->WP_Widget('popcat-widget', __('Custom Most Popular Categories', 'youare'), $widget_ops, $control_ops);

    $this->defaults = array('catlist_title' => __('Topics', 'youare'),
        'catlist_show_count' => __(1, 'youare'),
        'catlist_order' => __(1, 'youare'),
        'catlist_limit' => __(10, 'youare'),
        'catlist_dropdown' => __(0, 'youare'),
        'catlist_cat_exc' => __('', 'youare'),
        'catlist_exclude_zero' => 1,
        'catlist_cache' => __('', 'youare')
    );
  ...
  function form($instance) {
    ...
    <p>
      <label for="<?php echo $this->get_field_id('catlist_title'); ?>"><?php _e('Title:', 'youare'); ?></label>
    ...
    <p>
      ...
      <label for="<?php echo $this->get_field_id('catlist_show_count'); ?>"><?php _e('Display Count', 'youare'); ?></label>
    </p>

    <p>
      <label for="<?php echo $this->get_field_id('catlist_order'); ?>"><?php _e('Category List order:', 'youare'); ?></label>
      <select id="<?php echo $this->get_field_id('catlist_order'); ?>" name="<?php echo $this->get_field_name('catlist_order'); ?>">
        <option value="1"<?php echo ($instance['catlist_order'] == 1) ? ' selected="selected"' : ''; ?>><?php _e('Most Popular', 'youare'); ?></option>
        <option value="2"<?php echo ($instance['catlist_order'] == 2) ? ' selected="selected"' : ''; ?>><?php _e('Alphabetical', 'youare'); ?></option>
      </select>
    </p>

    <p>
      <label for="<?php echo $this->get_field_id('catlist_limit'); ?>"><?php _e('Category Limit:', 'youare'); ?></label>
    ...
    <p>
      <label for="<?php echo $this->get_field_id('catlist_cat_exc'); ?>"><?php _e('Exclude Category by ID:', 'youare'); ?></label>
      ...
      <span style="font-size:.9em;"><?php _e('Separate IDs by comma.', 'youare'); ?></span>
    </p>
    ...
    <p>
      ...
      <label for="<?php echo $this->get_field_id('catlist_exclude_zero'); ?>"><?php _e('Exclude Categories with 0 posts', 'youare'); ?></label>
    </p>
    ...
    <p>
      ...
      <label for="<?php echo $this->get_field_id('catlist_dropdown'); ?>"><?php _e('Display as a drop down', 'youare'); ?></label>
    </p>
