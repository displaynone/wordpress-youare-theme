...
jQuery(document).ready(function() {
  jQuery('#edit-thumb').live('click', function() {
    var html = '<img id="you_thumb_crop" src="'+jQuery('img.attachment-post-thumbnail').attr('src')+'" style="padding: 0px; margin: 0px;" />';
    console.log(html);
    ...
    <?php if ($data) { ?>    
    crop = jQuery.Jcrop('#you_thumb_crop', {aspectRatio: 1, minSize: [150, 150], setSelect: sel});
    <?php } else { ?>
    crop = jQuery.Jcrop('#you_thumb_crop', {aspectRatio: 1, minSize: [150, 150]});
    <?php } ?>
    return false;
  });
});
